private _no =_this select 0;
private _sideEntered = (_this select 1) call smm_fnc_macrosFromConfigSide;
...
if!(zoneActive select _no) then{
    ...
    [_no,_sideEntered] spawn {
        private _no          = _this select 0;
        private _sideEntered = _this select 1;
        private _zone        = _no call getZone;
        
        private _pos = _zone call smm_fnc_getPosition;
        private _size = _zone call smm_fnc_getSize;
        private _zoneSide = _zone call smm_fnc_getSide;
        if(_zoneSide != _sideEntered && _zoneSide != civilian )then{
            zoneActive set [_no,true];
            private _marker = _no call smm_fnc_getHash;
            ...
            private _currentGroup = createGroup _zoneSide;
			[_pos,60,"ColorBlue"] call smm_fnc_createDebugMarker;
            private _randGroupPos = [_pos,(_size * 2)/3] call getPosNear;
            {
				...
				private _randPos = [_randGroupPos,_size/3] call getPosNear;
				...
				private _unit =  (( units _currentGroup )select ((count (units _currentGroup))-1)) ;
            ...
            private _buildings = (zoneNoToBuildings select _no);
            if((count _buildings)> (count _allGroups) )then{
            {
                private _building = _x select 0;
                private _buildingPos = _x select 1;
                if(_forEachIndex < (count _allGroups))then{
                    ...
                    private _positions = [count _units,_buildingPos] call smm_fnc_getMultipleRandom;
                    {
            ...
            private _vehicles = [_vehicleMoney,_zoneSide] call getVehicles;
            ...
            private _allVehicles = [];
            {
                ...
                private _veh = [_x,_randPos] call smm_fnc_createVehicle;
				[_veh,_randPos] call smm_fnc_setPosAGLS;
                [_no,_veh] call smm_fnc_addVehicle;
                private _vehUnits = [_zoneSide,_veh,_currentGroup] call smm_fnc_fillVehicle;
            ...
            private _grp = _x;
            private _startWP = _x addWaypoint [_pos,0];
