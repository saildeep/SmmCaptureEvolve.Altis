/*
	Author: Karel Moricky modified by Fezward
...
switch _mode do {
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	case "Open": {
		private _fncName = "Arsenal 'Open'"; LOGFNCCALL
		
		if !(uinamespace getVariable ["smm_fnc_virtualshop_init",false]) then {
			[] call compile preprocessFile "smm_virtualshop\functions\init.sqf";
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	case "Init": {
		private _fncName = "Arsenal 'Init'"; LOGFNCCALL
		...
		INITTYPES
		["InitGUI",[_display,"smm_fnc_virtualshop"]] call smm_fnc_virtualshop;
		["Preload"] call smm_fnc_virtualshop;
		["ListAdd",[_display]] call smm_fnc_virtualshop;
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	case "InitGUI": {
		private _fncName = "Arsenal 'InitGUI'"; LOGFNCCALL
		...
		if (is3DEN) then {
			...
			_center = if (_function == "smm_fnc_virtualshop") then {
			...
			if (_function == "smm_fnc_virtualshop") then {
		...
		_display displayaddeventhandler ["mousebuttondown","with uinamespace do {['MouseButtonDown',_this] call smm_fnc_virtualshop;};"];
		_display displayaddeventhandler ["mousebuttonup","with uinamespace do {['MouseButtonUp',_this] call smm_fnc_virtualshop;};"];
		//_display displayaddeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call smm_fnc_virtualshop;};"];
		_display displayaddeventhandler ["keydown","with (uinamespace) do {['KeyDown',_this] call smm_fnc_virtualshop;};"];
		//_display displayaddeventhandler ["mousemoving","with (uinamespace) do {['Loop',_this] call smm_fnc_virtualshop;};"];
		//_display displayaddeventhandler ["mouseholding","with (uinamespace) do {['Loop',_this] call smm_fnc_virtualshop;};"];

		_ctrlMouseArea = _display displayctrl IDC_RSCDISPLAYARSENAL_MOUSEAREA;
		_ctrlMouseArea ctrladdeventhandler ["mousemoving","with uinamespace do {['Mouse',_this] call smm_fnc_virtualshop;};"];
		_ctrlMouseArea ctrladdeventhandler ["mouseholding","with uinamespace do {['Mouse',_this] call smm_fnc_virtualshop;};"];
		_ctrlMouseArea ctrladdeventhandler ["mousebuttonclick","with uinamespace do {['TabDeselect',[ctrlparent (_this select 0),_this select 1]] call smm_fnc_virtualshop;};"];
		_ctrlMouseArea ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call smm_fnc_virtualshop;};"];
		...
		_ctrlButtonInterface ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonInterface',[ctrlparent (_this select 0)]] call smm_fnc_virtualshop;};"];
		...
		_ctrlButtonSave ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSave',[ctrlparent (_this select 0)]] call smm_fnc_virtualshop;};"];

		_ctrlButtonLoad = _display displayctrl IDC_RSCDISPLAYARSENAL_CONTROLSBAR_BUTTONLOAD;
		_ctrlButtonLoad ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonLoad',[ctrlparent (_this select 0)]] call smm_fnc_virtualshop;};"];
		...
		_ctrlArrowLeft ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),-1]] call smm_fnc_virtualshop;};"];

		_ctrlArrowRight = _display displayctrl IDC_RSCDISPLAYARSENAL_ARROWRIGHT;
		_ctrlArrowRight ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),+1]] call smm_fnc_virtualshop;};"];
		...
		_ctrlTemplateButtonCancel ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateCancel',[ctrlparent (_this select 0)]] call smm_fnc_virtualshop;};"];

		_ctrlTemplateButtonDelete = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_BUTTONDELETE;
		_ctrlTemplateButtonDelete ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateDelete',[ctrlparent (_this select 0)]] call smm_fnc_virtualshop;};"];

		_ctrlTemplateValue = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_VALUENAME;
		_ctrlTemplateValue ctrladdeventhandler ["lbselchanged","with uinamespace do {['templateSelChanged',[ctrlparent (_this select 0)]] call smm_fnc_virtualshop;};"];
		...
		if !(isnull _ctrlIcon) then {
...

			{
				...
				_mode = if (_idc in [IDCS_LEFT]) then {"TabSelectLeft"} else {"TabSelectRight"};
				{
					...
					_x ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call smm_fnc_virtualshop;};"];
				} foreach [_ctrlIcon,_ctrlTab];

				_sort = _sortValues param [_idc,0];
				_ctrlSort = _display displayctrl (IDC_RSCDISPLAYARSENAL_SORT + _idc);
				_ctrlSort ctrladdeventhandler ["lbselchanged",format ["with uinamespace do {['lbSort',[_this,%1]] call smm_fnc_virtualshop;};",_idc]];
				...
				_ctrlList ctrladdeventhandler ["lbdblclick",format ["with uinamespace do {['ShowItem',[ctrlparent (_this select 0),(_this select 0),%1]] spawn smm_fnc_virtualshop;};",_idc]];
		...
		['TabDeselect',[_display,-1]] call smm_fnc_virtualshop;
		...
		_ctrlButtonClose ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonClose',[ctrlparent (_this select 0)]] spawn smm_fnc_virtualshop;}; true"];
		...
		if (_fullVersion && !is3DEN) then {
			if (missionname == "Arsenal") then {
				...
				_ctrlSpace ctrlshow true;
				{
					...
					_ctrl ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSpace',_this] spawn smm_fnc_virtualshop;}; true"];
		...
		//--- Camera reset
		["Mouse",[controlnull,0,0]] call smm_fnc_virtualshop;
		BIS_fnc_arsenal_draw3D = addMissionEventHandler ["draw3D",{with (uinamespace) do {['draw3D',_this] call smm_fnc_virtualshop;};}];

		setacctime (missionnamespace getvariable ["BIS_fnc_arsenal_acctime",1]);
		
		CTRL_BUTTON_OK ctrlenable true;
	};

	///////////////////////////////////////////////////////////////////////////////////////////
	case "Preload": {
		private _fncName = "Arsenal 'Preload'"; LOGFNCCALL
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	case "Exit": {
		private _fncName = "Arsenal 'Exit'"; LOGFNCCALL
...

		{
			...
			switch _foreachindex do {
				...
				case IDC_RSCDISPLAYARSENAL_TAB_CARGOPUT: {
					...
					_virtualAll = _fullVersion || {"%ALL" in _virtualCargo};
					_ctrlList lnbSetColumnsPos [0.07,0.15,0.60,0.70];
					_columns = count lnbGetColumnsPosition _ctrlList;
					{
						if (_virtualAll || {_x in _virtualCargo}) then {
							_xCfg = configfile >> "cfgmagazines" >> _x;
							_lbAdd = _ctrlList lnbaddrow ["",gettext (_xCfg >> "displayName"),str 0,ADDPRICETAGLEFT];
			...
			if (ctrltype _ctrlList == 102) then {
				...
				_sortValues = uinamespace getvariable ["bis_fnc_arsenal_sort",[]];
				["lbSort",[[_ctrlSort,_sortValues param [_foreachindex,0]],_foreachindex]] call smm_fnc_virtualshop;
			};
		} foreach _data;
	};
	...
	case "ListSelectCurrent": {
		...
		_defaultShow = uinamespace getvariable ["bis_fnc_arsenal_defaultShow",-1];
		{
			...
			_current = if (count _defaultItem == 0) then {
				...
				if (_defaultShow < 0) then {["ShowItem",[_display,_ctrlList,_foreachindex]] spawn smm_fnc_virtualshop;};
		...
		if (_defaultShow >= 0) then {["ShowItem",[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _defaultShow),_defaultShow]] spawn smm_fnc_virtualshop;};
	...
	case "TabDeselect": {
		...
		if ({count _x > 0} count BIS_fnc_arsenal_buttons == 0) then {
			...
			if (!_shown || _key == 1) exitwith {['buttonInterface',[_display]] call smm_fnc_virtualshop;};
	...
	case "TabSelectLeft": {
...

		{
			...
			if (_active) then {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _idc),_idc]] call smm_fnc_virtualshop;
			};
		...
		if (_showItems) then {['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC]] call smm_fnc_virtualshop;};
		...
		_ctrl ctrlsetfade _fadeCargo;
		_ctrl ctrlcommit FADE_DELAY;
		if (_showCargo) then {['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_CARGOMAG]] call smm_fnc_virtualshop;};
		...
		//['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _index),_index]] call smm_fnc_virtualshop;
	};
		...
		_ctrBackgroundRight = _display displayctrl IDC_RSCDISPLAYARSENAL_BACKGROUNDRIGHT;
		{
			...
			if (_active) then {
				...
				if (
					...
					["SelectItemRight",[_display,_ctrlList,_index]] call smm_fnc_virtualshop;
				};
			};
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	case "SelectItem": {
		private _fncName = "Arsenal 'SelectItem'"; LOGFNCCALL
		...
		switch _index do {
			case IDC_RSCDISPLAYARSENAL_TAB_UNIFORM: {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA),IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA]] spawn smm_fnc_virtualshop;
			};
			case IDC_RSCDISPLAYARSENAL_TAB_VEST: {
				if (_item == "") then {
					removevest _center;
					...
					_cfgMuzzle = if (_x == "this") then {_cfgWeapon} else {_cfgWeapon >> _x};
					{
						...
						if (CONDITION(_virtualMagazineCargo)) then {
							...
							if !(_mag in _magazines) then {
								...
								_lbAdd = _ctrlList lnbaddrow ["",_displayName,str _value,ADDPRICETAGLEFT];
		...
		if (
			...
			_weaponAccessories = _center weaponaccessories _weapon;
			{
				...
				["lbSort",[[_ctrlSort,lbcursel _ctrlSort],_x]] call smm_fnc_virtualshop;
			} foreach [
				IDC_RSCDISPLAYARSENAL_TAB_ITEMMUZZLE,
				IDC_RSCDISPLAYARSENAL_TAB_ITEMACC,
				IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC,
		...
		if (ctrlenabled _ctrlList) then {
			...
			if (BIS_fnc_arsenal_type == 0 || (BIS_fnc_arsenal_type == 1 && !is3DEN)) then {
				["ShowItemInfo",[_itemCfg]] call smm_fnc_virtualshop;
				["ShowItemStats",[_itemCfg]] call smm_fnc_virtualshop;
			};
		};
	...
	case "SelectItemRight": {
		private _fncName = "Arsenal 'SelectItemRight'"; LOGFNCCALL
		...
		switch true do {
			...
			case (_key in [DIK_RETURN,DIK_NUMPADENTER]): {
				...
				if (ctrlfade _ctrlTemplate == 0) then {
					if (BIS_fnc_arsenal_type == 0) then {
						["buttonTemplateOK",[_display]] spawn smm_fnc_virtualshop;
					} else {
			...
			case (_key == DIK_TAB): {
				...
				if (BIS_fnc_arsenal_type == 0) then {
					["TabSelectLeft",[_display,_idc]] call smm_fnc_virtualshop;
				} else {
			...
			case (_key == DIK_C): {
				...
				if (BIS_fnc_arsenal_type == 0) then {
					if (_ctrl) then {['buttonExport',[_display,_mode]] call smm_fnc_virtualshop;};
				} else {
			...
			case (_key == DIK_V): {
				if (BIS_fnc_arsenal_type == 0) then {
					if (_ctrl) then {['buttonImport',[_display]] call smm_fnc_virtualshop;};
				} else {
			...
			//--- Save
			case (_key == DIK_S): {
				if (_ctrl) then {['buttonSave',[_display]] call smm_fnc_virtualshop;};
			};
			//--- Open
			case (_key == DIK_O): {
				if (_ctrl) then {['buttonLoad',[_display]] call smm_fnc_virtualshop;};
			};
			//--- Randomize
			case (_key == DIK_R): {
				if (_ctrl) then {
					if (BIS_fnc_arsenal_type == 0) then {
						if (_shift) then {
							...
							['buttonRandom',[_display]] call smm_fnc_virtualshop;
						};
					} else {
			...
			case (_key == DIK_BACKSPACE && !_inTemplate): {
				['buttonInterface',[_display]] call smm_fnc_virtualshop;
				_return = true;
			};
...
/*
			...
			case (_key == DIK_DELETE): {
				...
				if !(ctrlenabled _ctrlMouseBlock) then {
					['buttonTemplateDelete',[_display]] call smm_fnc_virtualshop;
					_return = true;
				};
			};
*/
	...
	case "buttonTemplateDelete": {
		...
		['showTemplates',[_display]] call (uinamespace getvariable (["smm_fnc_virtualshop","bis_fnc_garage"] select BIS_fnc_arsenal_type));
		_ctrlTemplateValue lnbsetcurselrow (_cursel max (lbsize _ctrlTemplateValue - 1));

		["templateSelChanged",[_display]] call smm_fnc_virtualshop;
/*
...
*/
	...
	case "showTemplates": {
		...
		["templateSelChanged",[_display]] call smm_fnc_virtualshop;
		//['buttonExport',[_display]] call smm_fnc_virtualshop;
	};
	...
	case "buttonImport": {
		...
		if (count _disabledItems > 0) then {
			['showMessage',[_display,localize "STR_A3_RscDisplayArsenal_message_unavailable"]] call smm_fnc_virtualshop;
		};
	...
	case "buttonExport": {
		...
		['showMessage',[_display,localize "STR_a3_RscDisplayArsenal_message_clipboard"]] call smm_fnc_virtualshop;
	};
...
*/
	...
	case "buttonSave": {
		_display = _this select 0;
		['showTemplates',[_display]] call (uinamespace getvariable (["smm_fnc_virtualshop","bis_fnc_garage"] select BIS_fnc_arsenal_type));//smm_fnc_virtualshop;
		...
		['showMessage',[_display,localize "STR_A3_RscDisplayArsenal_message_save"]] call smm_fnc_virtualshop;
	};
	...
	case "buttonSpace": {
		...
		_function = ["smm_fnc_virtualshop","bis_fnc_garage"] select _buttonID;
	...
	case "buttonOK": {
		...
		if !(_currentInvalidItems isEqualTo []) exitWith {
			private _formatedInvalidItems = _currentInvalidItems select 0;
			{
				if (count _formatedInvalidItems < 50) then {
					_formatedInvalidItems = _formatedInvalidItems + ", " + _x;
				} else {
					if (_foreachindex + 2 == count _currentInvalidItems) then {
						_formatedInvalidItems = _formatedInvalidItems + ", ...";
					};
				};
		...
		[[true, false] call smm_fnc_virtualshop_listInventory] call smm_fnc_virtualshop_setInventory;
		_display closedisplay 2;
	...
	case "AmmoboxInit": {
		...
		[["AmmoboxServer",_box,true],"smm_fnc_virtualshop",false] call bis_fnc_mp;
	};
	///////////////////////////////////////////////////////////////////////////////////////////
	case "AmmoboxExit": {
		private ["_box"];
		_box = [_this,0,objnull,[objnull]] call bis_fnc_param;
		[["AmmoboxServer",_box,false],"smm_fnc_virtualshop",false] call bis_fnc_mp;
	};
	///////////////////////////////////////////////////////////////////////////////////////////
	case "AmmoboxServer": {
		_box = [_this,0,objnull,[objnull]] call bis_fnc_param;
		...
		["AmmoboxLocal","smm_fnc_virtualshop",true,isnil "bis_fnc_arsenal_ammoboxServer"] call bis_fnc_mp;
			...
			if (isnil {_x getvariable "bis_fnc_arsenal_action"}) then {
				_action = _x addaction [
					localize "STR_A3_Arsenal",
					{
						...
						["Open",[nil,_box,_unit]] call smm_fnc_virtualshop;
					},
					[],
					6,
					true,
